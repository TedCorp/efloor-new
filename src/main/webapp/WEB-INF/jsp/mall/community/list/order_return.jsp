					...
					<tr>
						<c:choose>
	            	 		<c:when test="${empty list.BRD_PW }">
	       	    				<td style="text-align: left;"><a href="${contextPath}/community/orderReturn/detail/view?pageNum=${obj.pageNum }&rowCnt=${obj.rowCnt }&BRD_NUM=${list.BRD_NUM }">${list.BRD_SBJT }</a>
	   						</c:when>
	   						<c:when test="${!empty list.BRD_PW && USER.MEMB_ID eq list.WRTR_ID}">
								...
								<a href="${contextPath}/community/orderReturn/detail/view?pageNum=${obj.pageNum }&rowCnt=${obj.rowCnt }&BRD_NUM=${list.BRD_NUM }">${list.BRD_SBJT }</a>
							</c:when>
...
function insBtnQna(){
	...
	location.href ='${contextPath }/community/orderReturn/detail?BRD_NUM=';   
}
