	...
	$(function() {
		...
		$('#sit_btn_cart').click(function(){
        	...
        	<c:if test="${empty USER.MEMB_ID}">
	    		...
	    		//비회원 장바구니
        		var strPdCode = "${result.PD_CODE }";
        		var strPdQty = $('#CT_QTY').val();
        	    ...
        	    if(strCookieBasket != undefined) {
            		...
            		$.each(arrCookieBasket, function(key, line) {
            		    var data = line.split('#');
            		    if(data[0] == strPdCode){
            		    	bConfirm = confirm("장바구니에 이미 등록된 상품 입니다. 장바구니로 이동 하시겠습니까?");
            		...
            		if(bChk){
            			strCookieBasket +=  "$" + strPdCode + "#" + strPdQty;
                		$.cookie('guestBasket',strCookieBasket,{
		                            expires : 7
		                           ,path : "/"
		                });
                		
        		    	bConfirm = confirm("장바구니에 등록되었습니다. 장바구니로 이동 하시겠습니까?");
            		}
...
<div id="sit_hhtml"></div>
	...
	<div id="sit">
		        ...
		        <div id="mslider" class="flexslider">
					...
					<c:if test="${ !empty(fileList) }">
						<c:forEach var="var" items="${ fileList }" varStatus="status">
							<li><a class="popup_item_image"><img src="${contextPath }/common/commonFileDown?ATFL_ID=${var.ATFL_ID }&ATFL_SEQ=${var.ATFL_SEQ}" width="400" height="450" class="goodsImg" alt=""></a></li>
						</c:forEach>
					</c:if>
					<c:if test="${ empty(fileList) }">
						<li><a class="popup_item_image"><img src="${contextPath }/resources/images/mall/goods/noimage.png" width="450" height="400" class="goodsImg" alt=""></a></li>
					</c:if>
			       </ul>
		        </div>
		    </div>
		...
		<section id="sit_ov">
			<c:if test="${result.SALE_CON eq 'SALE_CON_02' }">
				<h2 id="sit_title" style="color:red">[품절]</h2>
			</c:if>
			<h2 id="sit_title"><c:out value="${ result.PD_NAME }" escapeXml="true"/></h2>
			<div class="sit_pr">
				<h3>판매가격</h3>
				<c:if test="${ result.REAL_PRICE ne result.PD_PRICE }">
					<strike><fmt:formatNumber value="${ result.PD_PRICE }" pattern="#,###"/>원</strike><br>
				</c:if>
				<fmt:formatNumber value="${ result.REAL_PRICE }" pattern="#,###"/>원
			...
			<li class="dt_dv">
				<div id="sit_tot_price">총 금액 : <fmt:formatNumber value="${ result.REAL_PRICE }" pattern="#,###"/>원</div>
			</li>
	...
	$(function(){
	    ...
	    $("#btnList").click(function() {
	    	...
	    	setTimeout("fnListMove()", 500);
	    });
	        
	});
	// 상세페이지로 바로 왔을 경우 0.5초후에 목록으로 이동
	function fnListMove(){
