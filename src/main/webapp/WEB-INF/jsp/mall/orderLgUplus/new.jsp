...
$(function() {
    ...
    $("#btnSave").click(function() {
		...
		if(!$("#agree").prop("checked")) {
            alert("청약철회 관련방침의 내용에 동의하셔야 상품을 구매 하실 수 있습니다.");
            $("#agree").focus();
            return false;
		}
...
function fn_dlarGubn(){
	...
	//alert($("input:radio[name='DLAR_GUBN']:checked").val());
	...
	<tbody>
		...
		<tr>
			...
			<c:if test="${ tot_amt < supplierInfo.DLVA_FCON }">
				...
				<fmt:formatNumber value="${devy_amt }" /><br/>
				<select name="DAP_YN" id="DAP_YN">
					<option value="Y">배송비 결제</option>
					<option value="N">배송비 착불</option>
				</select>
			</c:if>
			<c:if test="${ tot_amt >= supplierInfo.DLVA_FCON }">
				0<br/>
...
<div class="mbskin">
    ...
    <section id="fregister_term">
        <textarea readonly style="width:730px; height:150px">
제1조(목적)
   
이 약관은 회사(전자거래 사업자)가 운영하는 재식자 사이버 몰(이하 "몰"이라 한다)에서 제공하는 인터넷 관련 서비스(이하 "서비스"라 한다)를 이용함에 있어 사이버몰과 이용자의 권리·의무 및 책임사항을 규정함을 목적으로 합니다. 
※ 「PC통신등을 이용하는 전자거래에 대해서도 그 성질에 반하지 않는한 이 약관을 준용합니다」 
 
 
제2조(정의)
   
① "몰"이란 재식자 회사가 재화 또는 용역을 이용자에게 제공하기 위하여 컴퓨터등 정보통신설비를 이용하여 재화 또는 용역을 거래할 수 있도록 설정한 가상의 영업장을 말하며, 아울러 사이버몰을 운영하는 사업자의 의미로도 사용합니다.  
...
1. 이 약관은 2016년 1월 1일부터 적용됩니다.
        </textarea>
        
		<div class="checkbox">
		  <label>
		    <input type="checkbox" name="agree" id="agree" value="1">
		    청약철회 관련방침의 내용에 동의합니다.
		  </label>
		</div>
