...
$(function(){
    ...
    $(".btnExcelDownload").click(function() {    	
    	...
    	//form.target = "_blank";
    	form.method = "post";
    	form.action = "${contextPath }/adm/productRcmdMgr/excelDownRetail";
...
function fn_pd_popup(){
	window.open("${contextPath }/adm/productRcmdMgr/popupRetail", "_blank", "width=900,height=900"); 
}
...
function fn_pd_multi_return(checkboxValues){
	...
	for(var i=0; i<checkboxValues.length; i++){
		...
		//그루핑 중복조회	
		$.ajax({
		    url: "${contextPath }/adm/productRcmdMgr/grpRtChk",
		    type: 'post',
	...
	<spform:form name="rcmdFrm" id="rcmdFrm" method="${strMethod }" action="${strActionUrl }" class="form-horizontal">
		...
		<div class="box box-info">
								...
								<div class="box-body">
									<table class="table table-bordered">
										...
										<c:if test="${errlist.size() == 0}">
											<tr>
												<td colspan="4"><label>조회된 에러내용이 없습니다.</label></td>
											</tr>
										</c:if>
										</tbody>
									</table>
			...
			<div class="box-footer" >
				<a href="${contextPath}/adm/productRcmdMgr" class="btn btn-default pull-right" style="margin-left:5px;">리스트</a>
				<a onclick="javascript:fn_save();" class="btn btn-primary pull-right" style="margin-left:5px;">저장</a>
				<c:if test="${ !empty tb_pdrcmdxm.GRP_CD }"><!-- 수정시 사용 -->
					<a onclick="javascript:fn_delete();" class="btn btn-danger pull-right" style="margin-left:5px;">삭제</a>
				</c:if>
			</div>			
			<!-- /.box-footer -->
	<!-- /.box -->
