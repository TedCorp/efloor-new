...
function fn_delete(){
	...
	var frm=document.getElementById("rcmdFrm");
	frm.action = "${contextPath }/adm/entryCategoryMgr/delete/${entCagoInfo.ENTRY_ID}";	
	frm.submit();
}

//상품선택 팝업 호출
function fn_pd_popup(){
	window.open("${contextPath }/adm/entryCategoryMgr/popup", "_blank", "width=900,height=900"); 
}
...
<c:set var="strActionUrl" value="${contextPath }/adm/entryCategoryMgr" />
<c:set var="strMethod" value="post" />
<c:if test="${ !empty entCagoInfo.ENTRY_ID }"><!-- 수정시 사용 -->
	<c:set var="strActionUrl" value="${contextPath }/adm/entryCategoryMgr/${entCagoInfo.ENTRY_ID }" />
			...
			<div class="box-body">
				<div class="form-group">
					...
					<div class="form-group">
						...
						<div class="col-sm-8">
							<input class="form-control" type="text" id="ENTRY_NAME" name="ENTRY_NAME" value="${entCagoInfo.ENTRY_NAME }" maxlength="50" />
						</div>
					</div><br/>
					<div class="form-group">
						...
						<div class="col-sm-3">
							<input class="form-control input-sm number" type="text" 
								id="SORT_ORDRM" name="SORT_ORDRM" value="${entCagoInfo.SORT_ORDR }" maxlength="5"/>
						</div>
