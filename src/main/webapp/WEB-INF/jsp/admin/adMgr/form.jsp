		...
		<div class="box box-info">
			...
			<div class="box-body">
				<div class="form-group">
					...
					<input type="hidden" id="ATFL_ID" name="ATFL_ID" value="${adverinfo.ATFL_ID }">	
					
					<label class="col-sm-2 control-label">광고번호</label>
						<div class="col-sm-8">
							<c:if test="${ !empty adverinfo.AD_ID }">
								<input type="text" class="form-control" id="AD_ID" name="AD_ID" value="${adverinfo.AD_ID }" readonly="readonly" />
							</c:if>
							<c:if test="${ empty adverinfo.AD_ID }">
				...
				<c:if test="${ !empty adverinfo.AD_ID }">
					<div class="form-group">
						...
						<div class="col-sm-8">
							<p class="form-control-static">${currentUrl }/adNew?AD_ID=${adverinfo.AD_ID }</p>
				...
				<div class="form-group">
					...
					<div class="col-sm-8">
						<input type="text" class="form-control" id="AD_NAME" name="AD_NAME" value="${adverinfo.AD_NAME }" placeholder="광고명" />
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input type="text" class="form-control" id="START_DT" name="START_DT" value="${adverinfo.START_DT }" placeholder="예) 2017.02.06(월)" style="width:150px;" />
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-10">
						<input
							type="text" class="form-control" id="END_DT" name="END_DT" value="${adverinfo.END_DT }" placeholder="예) 02.11(토)" style="width:150px;" />
					</div>
				</div>
				<div class="form-group">
					...
					<div class="col-sm-10">
						...
						<c:if test="${ !empty adverinfo.AD_ID }">
							<c:set var="END_YN" value="${ adverinfo.END_YN }" />
						</c:if>
					...
					<div class="col-md-10">
						<div class="input-group">
							<label class="check" style="width:100%">
								<input type="radio" class="iradio" name="TOP_TYPE" value="1" ${ adverinfo.TOP_TYPE eq '1' ? 'checked' : '' } />
								<img src="${contextPath}/resources/img/sub/ad/header_type1.gif" alt=""/>
							</label>
							<label class="check" style="width:100%">
								<input type="radio" class="iradio" name="TOP_TYPE" value="2" ${ adverinfo.TOP_TYPE eq '2' ? 'checked' : '' } />
								<img src="${contextPath}/resources/img/sub/ad/header_type2.gif" alt=""/>
							</label>
							<label class="check" style="width:100%">
								<input type="radio" class="iradio" name="TOP_TYPE" value="3" ${ adverinfo.TOP_TYPE eq '3' ? 'checked' : '' } />
								<img src="${contextPath}/resources/img/sub/ad/header_type3.gif" alt=""/>
							</label>
							<label class="check" style="width:100%">
								<input type="radio" class="iradio" name="TOP_TYPE" value="0" ${ adverinfo.TOP_TYPE eq '0' ? 'checked' : '' } />
								이미지 직접 등록
							</label>
				...
				<div class="form-group" id="divTopFile">
					...
					<div class="col-md-3">
						<input type="hidden" name="TOP_ATFL" id="file-simple" value="${adverinfo.TOP_ATFL }">
						${adverinfo.TOP_ATFL }
				...
				<div class="form-group">
					...
					<div class="col-md-3">
						<input type="file" id="pdImgFile" multiple name="pdImgFile" id="file-simple" value="${adverinfo.TOP_ATFL }"> 예) 상품ID.jpg
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-md-8">
						<textarea name="AD_DESC" id="AD_DESC" class="form-control" rows="5">${adverinfo.AD_DESC } </textarea>
					...
					<table class="table table-bordered">
						<thead>
							<tr>
								...
								<th style="width:180px;">제약사항</th>
								<th style="width:120px;">할인&증정 명</th>
								<th style="width:90px;">할인&증정 가격</th>
								<th style="width:70px;">판매단위</th>
								<th style="width:50px;">정렬</th>
								<th style="width:55px;">첨부파일</th>
							</tr>
						</thead>
						<tbody id="pdtAdd">
							<c:forEach items="${detailList }" var="ent" varStatus="loop">
								<tr>
									...
									<td><input class="form-control input-sm" type="text" name="PD_CONSS" value="${ent.PD_CONS }"></td>
									
									<td><input class="form-control input-sm" type="text" name="OPT_NAMES" value="${ent.OPT_NAME }"></td>
									<td><input class="form-control input-sm number" type="text" name="OPT_PRICES" value="<fmt:formatNumber value="${ent.OPT_PRICE }" />"></td>
									<td><input class="form-control input-sm" type="text" name="UNIT_NAMES" value="${ent.UNIT_NAME }"></td>
									...
									<td>
										...
										<img class="goodsImg" src="${contextPath }/imgAdInfo?AD_ID=${ent.AD_ID }&PD_ID=${ent.PD_ID }&ATFL_ID=${adverinfo.ATFL_ID }" alt="<c:out value="${ ent.PD_NAME }" escapeXml="true"/>" width="50" height="50"/>
									</td>
								</tr>
							</c:forEach>
...
<script type="text/javascript">
	$(function() {
		...
		<c:if test="${ adverinfo.TOP_TYPE ne '0' }">
		...
		$(".btnView").click(function(){
		    window.open("${contextPath }/adNew?AD_ID=${adverinfo.AD_ID}&view=preview", "광고 미리보기", "width=1150, height=800, toolbar=no, menubar=no, scrollbars=yes, resizable=yes" );
        });
		
		$(".btnCopyPopup").click(function(){
			window.open("${contextPath }/adm/adMgr/popup?AD_ID_NCOPY=${adverinfo.AD_ID}", "_blank", "width=900,height=800"); 
		});
